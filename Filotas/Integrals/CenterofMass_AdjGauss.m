...
% Define the parameters for CoM_routine
e = -1;
f = 1;
tol = 1e-4;
inte3 = 0;

% Call CoM_routine repeatedly with different myfunc
for i = 1:4
    fprintf('Iteration %d\n', i);
    
    % Define a different myfunc for each iteration
    switch i
        case 1
            myfunc = @(x, y, z) 1+x^2 + 2*y^2 + 3*z^2;
        case 2
            myfunc = @(x, y, z) x*(1+x^2 + 2*y^2 + 3*z^2);
        case 3
            myfunc = @(x, y, z) y*(1+x^2 + 2*y^2 + 3*z^2);
        otherwise
            myfunc = @(x, y, z) z*(1+x^2 + 2*y^2 + 3*z^2);
    end
    
    % Call CoM_routine with the updated myfunc
    I(i)= CoM_routine1(e, f, tol, inte3, myfunc);
    ...
    sentence=sprintf('The center of mass is equal to (xcm,ycm,zcm)=(%.2f,%.2f,%.2f)',I(2),I(3),I(4));
    disp(sentence);
    
function rr=CoM_routine1(e,f,tol,inte3,myfunc)
